local classes = require("gpt4all.common.classes")
local Layout = require("nui.layout")
local Popup = require("nui.popup")

local ChatInput = require("gpt4all.input")
local Api = require("gpt4all.api")
local Config = require("gpt4all.config")
local Settings = require("gpt4all.settings")
local Sessions = require("gpt4all.flows.chat.sessions")
local Utils = require("gpt4all.utils")
local Signs = require("gpt4all.signs")
local Spinner = require("gpt4all.spinner")
local Session = require("gpt4all.flows.chat.session")
local SystemWindow = require("gpt4all.flows.chat.system_window")
...
function Chat:init()
  ...
  self.selected_message_nsid = vim.api.nvim_create_namespace("Gpt4AllNSSM")
...
function Chat:welcome()
  ...
  if #self.session.conversation == 0 or (#self.session.conversation == 1 and self.system_message ~= nil) then
    ...
    for line_num = 0, #lines do
      self:add_highlight("Gpt4AllWelcome", line_num, 0, -1)
    end
  end
  self:render_role()
end
...
function Chat:show_message_selection()
  ...
  vim.api.nvim_buf_set_extmark(self.chat_window.bufnr, self.selected_message_nsid, msg.start_line, 0, {
    ...
    hl_group = "Gpt4AllSelectedMessage",
    hl_eol = true,
  })
      ...
      virt_text = {
        {
          ...
          "Gpt4AllMessageAction",
        },
        { " ", "Gpt4AllSelectedMessage" },
      },
      virt_text_pos = "right_align",
    })
...
function Chat:renderLastMessage()
  ...
  if msg.type == QUESTION then
    for index, _ in ipairs(lines) do
      self:add_highlight("Gpt4AllQuestion", msg.start_line + index - 1, 0, -1)
    end
